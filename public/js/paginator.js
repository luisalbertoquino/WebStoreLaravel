...
function paginator(config) {
  ...
  if (_typeof(config) != "object") throw "Paginator was expecting a config object!";
  if (typeof config.get_rows != "function" && !(config.table instanceof Element)) throw "Paginator was expecting a table or get_row function!"; // get/set if things are disabled
  ...
  box = config.box; // get/make function for getting table's rows

  if (typeof config.get_rows != "function") {
    config.get_rows = function () {
      var table = config.table;
      var tbody = table.getElementsByTagName("tbody")[0] || table; // get all the possible rows for paging
  ...
  var trs = get_rows(); // get/set rows per page
  ...
  var rows_per_page = config.rows_per_page; // get/set current page

  if (typeof config.page == "undefined") {
    config.page = 1;
  }

  var page = config.page; // get page count

  var pages = rows_per_page > 0 ? Math.ceil(trs.length / rows_per_page) : 1; // check that page and page count are sensible values
  ...
  config.page = page; // hide rows not on current page and show the rows that are
  ...
  if (typeof config.box_mode == "function") {
    ...
    if (config.box_mode == "list") {
      make_button = function make_button(symbol, index, config, disabled, active) {
  ...
  stat.innerHTML = "On page " + page + " of " + pages + ", showing rows " + ((page - 1) * rows_per_page + 1) + " to " + (trs.length < page * rows_per_page || rows_per_page == 0 ? trs.length : page * rows_per_page) + " of " + trs.length;
  box.appendChild(stat); // hide pagination if disabled
...
/***/ 11:
/*!*****************************************!*\
  !*** multi ./resources/js/paginator.js ***!
  \*****************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\Users\alber\Desktop\WebStoreLaravel\resources\js\paginator.js */"./resources/js/paginator.js");
